        ...
        while (end_time - begin_time)<130:
            ...
            while  len(restBox)!=0:
                ...
                for box in restBox:
                    ...
                    # maxBox = max(box.width,box.length)
                    # minBox = min(box.width,box.length)
                    # maxCar = max(truck[chose_truck]['width'],truck[chose_truck]['length'])
                    # minCar = min(truck[chose_truck]['width'],truck[chose_truck]['length'])
                    # while box.height>=truck[chose_truck]['height'] or maxBox>=maxCar or minBox>=minCar:
                    #     #去掉这种类型的卡车
                    #     canUseTruck.remove(chose_truck)
                    #     #重新选一辆
                    #     next_truck = random.randint(0,len(canUseTruck)-1)
                    #     chose_truck = canUseTruck[next_truck]
                    #     maxCar = max(truck[chose_truck]['width'],truck[chose_truck]['length'])
                    #     minCar = min(truck[chose_truck]['width'],truck[chose_truck]['length'])
